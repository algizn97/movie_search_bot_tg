...
from state.states import LowBudget
from utils.paginator import Paginator

logging.config.dictConfig(LOGGING_CONFIG)
logger = logging.getLogger("low_budget_movie")

router = Router(name=__name__)


@router.message(Command("low_budget_movie"))
@router.message(F.text == "🔍 Поиск фильмов/сериалов с низким бюджетом")
async def cmd_low_budget_movie(message: types.Message, state: FSMContext) -> None:
    ...
    """
    try:
        await state.set_state(LowBudget.budget)
        await message.answer(
            text="Введите минимальный бюджет от 0 до 200 млн (например 1000-666666):",
            reply_markup=kbr.cancel,
        )
        logger.debug(
            "Command low_budget_movie processed successfully for user: %s",
            message.from_user.full_name,
        )
    except Exception as e:
        logger.error("Error processing command low_budget_movie: %s", e)


@router.message(LowBudget.budget)
async def process_budget(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if not (0 <= min_budget <= max_budget <= 200000000):
            await message.reply(
                "Пожалуйста, убедитесь, "
                "что бюджет находятся в диапазоне от 0 до 200 миллионов."
            )
            logger.warning(
        ...
        await state.set_state(LowBudget.ask_genre)
        logger.debug(
...
@router.message(LowBudget.ask_genre)
async def ask_genre_response(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if user_response == "да":
            ...
            await state.set_state(LowBudget.genre)
            ...
            await state.set_state(LowBudget.count)
            logger.info(
...
@router.message(LowBudget.genre)
...
@router.message(LowBudget.count)
async def process_count(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        movies = await low_budget_movie(budget, count, genre)
